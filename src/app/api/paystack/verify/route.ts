...
export async function GET(req: NextRequest) {
  ...
  if (!secretKey) {
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL}/e`
    );
  }
    ...
    try {
      ...
      const [created] = await db
        ...
        .values({
          ...
          email: metadata?.email || result.data.customer.email || 'no-email@example.com',
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL}/booking/success?ref=${reference}&bookingId=${bookingId}`
      );
    } catch (dbError) {
